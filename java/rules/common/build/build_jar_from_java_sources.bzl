...
def build_jar_from_java_sources(ctx):
    ...
    javac_args.add_joined(
        "-classpath",
        [dep[CustomJavaInfo].jar for dep in ctx.attr.deps],
        join_with = _UNIX_LIKE_CLASSPATH_SEPARATOR,
        omit_if_empty = True,
        map_each = _to_path,
    )
    javac_args.add_all(
    ...
    ctx.actions.expand_template(
        ...
        substitutions = {
            ...
            "{CLASS_OUTPUT_DIRECTORY}": ctx.attr.name + "_classes",
